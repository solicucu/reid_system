...
class InceptionB(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.stream1 = nn.Sequential(
            # 1x1 3x3_downsample but channel didn't double
            ConvBlock(in_channels, mid_channels, 1),
            ConvBlock(mid_channels, mid_channels, 3, s=2, p=1),
        )
        self.stream2 = nn.Sequential(
            # 1x1 3x3 3x3_downsample
            ConvBlock(in_channels, mid_channels, 1),
            ConvBlock(mid_channels, mid_channels, 3, p=1),
            ConvBlock(mid_channels, mid_channels, 3, s=2, p=1),
        )
...
class HACNN(nn.Module):
    ...
    def stn(self, x, theta):
        ...
        theta: (batch, 2, 3)
        """
        # given the cordinate theta, create a sample grid with the cordinate as center
    ...
    def forward(self, x):
        ...
        if self.learn_region:
            ...
            for region_idx in range(4):
                ...
                x1_trans_i = self.stn(x, x1_theta_i) # get the same size 
                # resize the image
...
if __name__ == "__main__":
    ...
    img = torch.randn(4, 3, 160, 64)
    model = HACNN(751, use_gpu = False)
